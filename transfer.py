...
def _get_extra_uv_lines(infofile): 
    ...
    *if you do not use model downloaded elsewhere, you do not need to use this function*
    """
...
def save_fbx(info_files): 
    ...
    for info_file in info_files: 
        ...
        if not osp.exists(info_file.replace(".txt", ".fbx")): 
            print("maya_save_fbx: some error occurred, fail to extract {}".format(info_file.replace(".txt", "*.fbx")))
            continue
        print('save to', info_file.replace(".txt", ".fbx"))

def get_info(human_pose, infoname, is_root_rotated=False):
    lines = open(infoname).readlines()
    meshname = infoname.replace(".txt", ".obj")
    inmesh = o3d.io.read_triangle_mesh(meshname)
    v_posed = np.array(inmesh.vertices)
...
if __name__ == '__main__':
    ...
    transfer_one_sequence("C:/Users/DELL/Desktop/groups/fbx/9837.txt", "info_seq_5.pkl")

    # for possible model downloaded online
    # clean_info("samples/Ch14_nonPBR.txt")
    #transfer_one_frame("C:/Users/DELL/Desktop/groups/fbx/455.txt", use_online_model=True)
